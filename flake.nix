{
  ...
  let
    ...
    # Workspace discovery
    ############################################################################
    mkPackageDirs = { basePath, filterFn ? (_: _: true), label }:
      let
        ...
        packagesAll =
          lib.foldl'
            (acc: vendor:
              ...
              in lib.attrsets.unionDisjoint acc pkgAttrs
            )
            {}
    ...
    rosLibsPath =
      let
        ...
        fmt = paths: lib.concatStringsSep ", " (map toString paths);
      in
        if found != null then found
        else throw "ROS libs directory not found; checked: ${fmt candidates}";
          ...
          in
            if hasUvLock then
              let
                ...
                depNames = builtins.filter (n: n != pkgName && !(builtins.elem n commonPackages))
...
EOF
        ...
        workspaceWithLaunch = pkgs.runCommand "${name}-with-launch" {} ''
          ...
          cp ${launchPath} $out/share/workspace.launch.py
        '';
  ...
  in
  {
    ...
    nixosConfigurations.rpi4 = nixpkgs.lib.nixosSystem {
      inherit system;
      specialArgs = {
        inherit pyEnv robotConsole robotApi rosWorkspace rosRuntimeEnv systemRosWorkspace systemRosRuntimeEnv;
      };
      modules = [
        ({ ... }: {
          nixpkgs.overlays =
